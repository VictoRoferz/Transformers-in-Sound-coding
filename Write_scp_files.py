...
SNR_all = [0, 5]
Noise_type_all = ["Noisy_noise-free-sound","Noisy_noise-sound-bible"]
Audio_sample = 10
...
for Noise_type in Noise_type_all:
    for SNR in SNR_all:
        for i in range(Audio_sample):
            ...
            noisy_path = rootdir + '/Train/Noisy/Noisy_' + Noise_type + '-' + str(SNR) + '_dB_' + str(i + 1) + '.wav'
...
for Noise_type in Noise_type_all:
    for SNR in SNR_all:
        for i in range(Audio_sample):
            ...
            noisy_path = rootdir + '/Dev/Noisy/Noisy_' + Noise_type + '-' + str(SNR) + '_dB_' + str(i + 1) + '.wav'
            L = noisy_path + " " + signal_path
            file.writelines(L)
            file.write('\n')
file.close()
...
for Noise_type in Noise_type_all:
    for SNR in SNR_all:
        for i in range(Audio_sample):
            ...
            noisy_path = rootdir + '/Test/Noisy/Noisy_' + Noise_type + '-' + str(SNR) + '_dB_' + str(i+1) + '.wav'
